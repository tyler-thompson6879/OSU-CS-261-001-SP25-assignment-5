 ...
 *
 * Name: Tyler Thompson
 * Email: Thompty2@oregonstate.edu
 */
...
/*
 * Helper function to count nodes in a subtree recursively
 */
...
/*
 * Helper function to compute height of a subtree recursively
 */
...
/*
 * Helper function to check path sums recursively
 */
int _bst_path_sum(int sum, struct bst_node* n, int current_sum) {
  if (n == NULL) {
    return 0;
  }
  ...
  return _bst_path_sum(sum, n->left, current_sum) || _bst_path_sum(sum, n->right, current_sum);
}
 ...
 */
void _push_left_nodes(struct stack* s, struct bst_node* n) {
  while (n != NULL) {
    stack_push(s, n);
    n = n->left;
  }
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  _push_left_nodes(iter->stack, bst->root);
...
int bst_iterator_next(struct bst_iterator* iter) {
  ...
  _push_left_nodes(iter->stack, current->right);
